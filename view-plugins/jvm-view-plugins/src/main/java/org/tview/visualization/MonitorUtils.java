...
public class MonitorUtils {
    ...
    public static JavaMemoryInfoEntity memoryInfo() throws IOException, MalformedObjectNameException {
        MemoryMXBean memoryMXBean = ManagementFactory.newPlatformMXBeanProxy(connection(), MEMORYNAME, MemoryMXBean.class);
        ...
        for (ObjectInstance objx : MBeanset2) {
            ...
            MemoryPoolMXBean memoryPoolMXBean = ManagementFactory.newPlatformMXBeanProxy(connection(), name, MemoryPoolMXBean.class);
     ...
     */
    public static List<JavaGcInfoEntity> gcInfoEntities() throws IOException, MalformedObjectNameException {
        ...
        for (ObjectInstance objx : MBeanset) {
            String name = objx.getObjectName().getCanonicalName();
            String keyName = objx.getObjectName().getKeyProperty("name");
            GarbageCollectorMXBean gc = ManagementFactory.newPlatformMXBeanProxy(platformMBeanServer, name, cls);
    ...
    public static JavaCpuEntity findCpuInfo() throws IOException {
        OperatingSystemMXBean os = ManagementFactory.newPlatformMXBeanProxy(connection(), OSBEANNAME, OperatingSystemMXBean.class);
    ...
    public static JavaDeadlockCheck deadlockCheck() throws IOException {
        ThreadMXBean threadMXBean = ManagementFactory.newPlatformMXBeanProxy(connection(), THREAD_BEAN_NAME, ThreadMXBean.class);
    ...
    public static JavaMonitorEntity jvmInfo() throws IOException {
        RuntimeMXBean runtimeMXBean = ManagementFactory.newPlatformMXBeanProxy(connection(), RUNTIMNAME, RuntimeMXBean.class);
        ClassLoadingMXBean classLoadingMXBean = ManagementFactory.newPlatformMXBeanProxy(connection(), CLSBEANAME, ClassLoadingMXBean.class);
        // 系统参数
